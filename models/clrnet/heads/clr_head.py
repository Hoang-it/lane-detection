...
class CLRHead(nn.Module):
    def __init__(self,
        ...
        self.cfg : dict = cfg
        self.img_w = self.cfg.get('img_w')
        self.img_h = self.cfg.get('img_h')
        ...
        self.criterion = torch.nn.NLLLoss(ignore_index=self.cfg.get('ignore_label'),
                                     weight=weights)
    ...
    def forward(self, x, **kwargs):
        ...
        if self.training:
            ...
            # print(f"self.priors {self.priors_on_featmap}")
        ...
        predictions_lists = []
        # print(f"predictions_lists {predictions_lists}")
        # iterative refine
        ...
        # print(f"predictions_lists {len(predictions_lists)}")
        if self.training:
            seg = None
            ...
            # print(f"seg_features {len(seg_features)}")
